    ...
    for review_data in dataset["full"]:
        ...
        if parent_asin in product_metadata_description:
            ...
            records_alpha[rating].append({
                "alpha": rating,
                "input": "Write a review for the product based on the title, category and description. Title: {}. Category: {} Description: {}".format(product[0], product[1], " ".join(description)),
                "target": review_text,
            })
...
dataset_dict.save_to_disk(DATASETS_FOLDER+"/amazon_review/multi_alpha_dataset_gemma")
